...
def getMatrix(docs, terms):
	...
	matrix = []
	for k in range(N):	
		i = keys[k]		#i is doc name
		l = []			#list l contains the idf values of all the terms for document i
		for j in terms:
			n = len(terms[j])	#n is df of the term j
			if i in terms[j]:
				l.append(log10(1+terms[j][i]) * log10(N/n))		#tf-idf calculation
			else:
				l.append(0)
...
def getResults(query):
	...
	q = []		# q is list of tf-idf values of all the terms in the query
	for i in terms:
		n = len(terms[i])		#n is df of i
